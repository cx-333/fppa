...
class VanillaGANLoss(nn.Module):
    def __init__(self, loss_weight: float, real_label: float=1.0, fake_label: float=0.0, loss_reduction: str='none'):
        super().__init__()
    ...
    def forward(self, x, is_real: bool, is_disc: bool=False, **kwargs):
        ...
        loss = self.loss(x, label).mean(dim=[1, 2, 3])
        return loss if is_disc else self.lamb_gan * loss        # （B，）
...
def calculate_adaptive_weight(nll_loss, g_loss, last_layer):
    ...
    nll_norm = torch.norm(nll_grads)
    g_norm = torch.norm(g_grads)

    # Guard against NaN/Inf in gradient norms (can happen early in training or when
    # the discriminator is too strong/weak).  Fall back to 1.0 so training continues.
    if torch.isnan(nll_norm) or torch.isnan(g_norm) or torch.isinf(nll_norm) or torch.isinf(g_norm):
        return torch.tensor(1.0, device=nll_loss.device)

    d_weight = nll_norm / (g_norm + 1e-4)
    d_weight = torch.clamp(d_weight, 0.0, 1e4).detach()
    return d_weight * 1.0   # lambda_adv_base 通常取 1.0
