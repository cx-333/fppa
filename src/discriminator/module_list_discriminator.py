...
from src.discriminator.multirate_clic21_gvae_discriminator import grouped_forward_by_rate_ind
...
class ModuleListDiscriminator(BaseDiscriminator):
    ...
    def forward(self, input, rate_ind: Union[float, Tensor], **kwargs):
        if isinstance(rate_ind, torch.Tensor) and rate_ind.numel() > 1:
            return grouped_forward_by_rate_ind(input, rate_ind, lambda r, x: self.subD_list[r](x, **kwargs))
