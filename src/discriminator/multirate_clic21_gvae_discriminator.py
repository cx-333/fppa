...
from src.discriminator.base_discriminator import BaseDiscriminator
...
def rate_ind_to_onehot_feat(image: Tensor, rate_ind: Union[int, float, Tensor], rate_level: int) -> Tensor:
    N, _, H, W = image.shape
    if isinstance(rate_ind, torch.Tensor) and rate_ind.numel() > 1:
        # batch 内多 rate_ind: (N,) -> (N, rate_level) -> (N, rate_level, H, W)
        one_hot = F.one_hot(rate_ind.long(), num_classes=rate_level).float().to(image.device)
        one_hot = one_hot.unsqueeze(-1).unsqueeze(-1).expand(-1, -1, H, W)
    else:
        if isinstance(rate_ind, torch.Tensor):
            rate_ind = rate_ind.item()
        one_hot = F.one_hot(torch.tensor([int(rate_ind)], dtype=torch.long, device=image.device), num_classes=rate_level).float()
        one_hot = one_hot.reshape(1, -1, 1, 1).repeat(N, 1, H, W)
    return one_hot


def grouped_forward_by_rate_ind(input: Tensor, rate_ind: Union[float, Tensor], forward_fn) -> Tensor:
    """按 rate_ind 分组执行前向传播并重组结果。
    
    当 rate_ind 为 (B,) 且 batch 内值不同时，按 unique rate 分组分别调用 forward_fn，
    最后按原始 batch 顺序重组输出。适用于 ModuleList 类 discriminator。
    
    Args:
        input: (B, C, H, W)
        rate_ind: scalar or (B,) tensor
        forward_fn: callable(rate_ind_scalar, sub_input) -> sub_output
    Returns:
        output: (B, ...)
    """
    if not isinstance(rate_ind, torch.Tensor):
        return forward_fn(int(rate_ind), input)
    if rate_ind.numel() == 1:
        return forward_fn(int(rate_ind.item()), input)
    
    B = input.size(0)
    assert rate_ind.numel() == B, f"rate_ind shape {rate_ind.shape} mismatch with batch size {B}"
    unique_rates = torch.unique(rate_ind, sorted=True).cpu().tolist()
    if len(unique_rates) == 1:
        return forward_fn(unique_rates[0], input)
    ...
    for r in unique_rates:
        mask = (rate_ind == r)
        sub_input = input[mask]
        sub_output = forward_fn(r, sub_input)
        if out_shape is None:
            out_shape = sub_output.shape[1:]
            out_dtype = sub_output.dtype
        outputs.append(sub_output)
        indices.append(mask.nonzero(as_tuple=True)[0])
    
    out = torch.empty(B, *out_shape, device=input.device, dtype=out_dtype)
    for idx, sub_out in zip(indices, outputs):
        out[idx] = sub_out
    return out
...
class SharedBackboneClic21GvaeDiscriminator(BaseDiscriminator):
    ...
    def forward(self, input, rate_ind: Union[float, Tensor], **kwargs):
        if isinstance(rate_ind, torch.Tensor) and rate_ind.numel() > 1:
            ...
            return grouped_forward_by_rate_ind(feat, rate_ind, lambda r, x: self.head_list[r](x))
...
class SharedHeadClic21GvaeDiscriminator(BaseDiscriminator):
    ...
    def forward(self, input, rate_ind: Union[float, Tensor], **kwargs):
        if isinstance(rate_ind, torch.Tensor) and rate_ind.numel() > 1:
            return grouped_forward_by_rate_ind(input, rate_ind, lambda r, x: self._single_forward(r, x))
        
        if isinstance(rate_ind, torch.Tensor):
            rate_ind = rate_ind.item()
        rate_ind = int(rate_ind)
        return self._single_forward(rate_ind, input)
    
    def _single_forward(self, rate_ind: int, input: Tensor) -> Tensor:
...
class MultirateSeparateClic21GvaeDiscriminator(BaseDiscriminator):
    ...
    def forward(self, input, rate_ind: Union[float, Tensor], **kwargs):
        if isinstance(rate_ind, torch.Tensor) and rate_ind.numel() > 1:
            return grouped_forward_by_rate_ind(input, rate_ind, lambda r, x: self.discriminator_list[r](x))
...
class MultirateSharedRateCondClic21GvaeDiscriminator(BaseDiscriminator):
    ...
    def forward(self, input, rate_ind: Union[float, Tensor], **kwargs):
        if isinstance(rate_ind, torch.Tensor):
            if rate_ind.numel() == 1:
                rate_ind = rate_ind.item()
        rate_ind = int(rate_ind) if not isinstance(rate_ind, torch.Tensor) else rate_ind
        one_hot = rate_ind_to_onehot_feat(input, rate_ind, self.rate_level)
        input = torch.cat([input, one_hot], dim=1)
