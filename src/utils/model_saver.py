...
class Saver(object):
    ...
    def save_best(self, network_dict: Dict, save_label: str) -> None:
        state_dict = {}
        for key, network in network_dict.items():
            state_dict[key] = network.state_dict()
        
        folder = self.path_hander.get_exp_path()
        save_path = os.path.join(folder, f"{save_label}.pth.tar")
        logger = get_root_logger()
        logger.debug(f'saving {save_label}  ("{save_path}")')
        torch.save(state_dict, save_path)
