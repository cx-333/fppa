...
class RDPCriterion(nn.Module):
    ...
    def compression_loss(self, x_ycbcr_hat, x_ycbcr, x_rgb_hat, x_rgb, d_fake):
        ...
        res["perceptual_loss"] = self.perceptual_loss(x_rgb_hat, x_rgb) * self.percept_weight
        
        res["adv_loss"] = self.gan_loss(d_fake, is_real=True, is_disc=False) * self.gen_weight 
...
def train_one_epoch(model, disc_model, criterion, optimizer, disc_optimizer, train_loader, val_loader, config, epoch,
    ...
    for batch_idx, d in enumerate(pbar):
        ...
        qp_index = torch.full((d.size(0),), 0, dtype=torch.long, device=device)
        ...
        d_fake = disc_model(x_rgb_hat, out_net["y_hat"])
        
        out_criterion = criterion.compression_loss(x_ycbcr_hat, x_ycbcr, x_rgb_hat, x_rgb, d_fake)
        ...
        d_real = disc_model(x_rgb, out_net["y_hat"].detach())
        d_fake = disc_model(x_rgb_hat.detach(), out_net["y_hat"].detach())
